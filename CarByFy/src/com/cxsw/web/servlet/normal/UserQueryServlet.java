...
public class UserQueryServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PageBean<User> userPage;
		try {
			userPage = userService.getPageBean(pageNumber, pageSize, key);
			request.setAttribute("userPage", userPage);
			request.setAttribute("key", key);
			//CookieUtil.addCookie(response, "operation_msg", "EDRTFGYHUJK");
			request.getRequestDispatcher("/beanclass/normal/userQuery.jsp").forward(request, response);
