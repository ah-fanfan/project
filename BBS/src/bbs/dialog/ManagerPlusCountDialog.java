...
public class ManagerPlusCountDialog extends Dialog {
	...
	protected Shell shell;
	  private  double[] ySeries = null;//纵轴上的值
	    ...
	    private String[] cagetorySeries = { "1级", "2级",
	            "3级", "4级", "5级" };//横轴
	...
	private void createContents() {
		...
		//1.设置shell布局方式
				...
				//2.查询数据
						List<Map<String, Object>> list1=db.query("SELECT u_chicek,COUNT(*) AS num FROM userlist WHERE u_chicek= '1级'  AND u_staus=1");
						List<Map<String, Object>> list2=db.query("SELECT u_chicek,COUNT(*) AS num FROM userlist WHERE u_chicek= '2级'  AND u_staus=1");
						List<Map<String, Object>> list3=db.query("SELECT u_chicek,COUNT(*) AS num FROM userlist WHERE u_chicek= '3级'  AND u_staus=1");
						List<Map<String, Object>> list4=db.query("SELECT u_chicek,COUNT(*) AS num FROM userlist WHERE u_chicek= '4级'  AND u_staus=1");
						List<Map<String, Object>> list5=db.query("SELECT u_chicek,COUNT(*) AS num FROM userlist WHERE u_chicek= '5级'  AND u_staus=1");
 ...
 public Chart createChart(Composite parent) {
        ...
        chart.getTitle().setText("会员等级统计表");
        ...
        chart.getAxisSet().getXAxis(0).getTitle().setText("等级类型");
