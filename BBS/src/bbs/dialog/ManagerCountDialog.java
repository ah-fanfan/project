...
public class ManagerCountDialog extends Dialog {
	...
	private void createContents() {
		...
		//1.设置shell布局方式
				...
				//2.查询数据
						List<Map<String, Object>> list1=db.query("SELECT t_type,COUNT(*) AS num FROM text WHERE  t_type='政治' AND t_staus=1");
						List<Map<String, Object>> list2=db.query("SELECT t_type,COUNT(*) AS num FROM text WHERE  t_type='军事' AND t_staus=1");
						List<Map<String, Object>> list3=db.query("SELECT t_type,COUNT(*) AS num FROM text WHERE  t_type='文化' AND t_staus=1");
						List<Map<String, Object>> list4=db.query("SELECT t_type,COUNT(*) AS num FROM text WHERE  t_type='科技' AND t_staus=1");
						List<Map<String, Object>> list5=db.query("SELECT t_type,COUNT(*) AS num FROM text WHERE  t_type='生活' AND t_staus=1");
	...
	public Chart createChart(Composite parent) {
        ...
        chart.getTitle().setText("尊敬的"+LoginDialog.name+"管理员，所有用户动态类型统计表如下");
